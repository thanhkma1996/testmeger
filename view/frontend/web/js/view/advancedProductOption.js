                            ...
                            if(row.special_price != 0 && row.apply_special_date_default == 1){
                                if (row.tiersi !== undefined) {
                                    ko.utils.arrayForEach(row.tiersi, function (tier) {
                                        ...
                                        if(type == 'percentage'){
                                            row.priceDisplay += " (If you buy from "+min+" to "+max+" products, you can purchase each product with this option for only "+symbol+price+symbol+" per product.)"
                                        }else {
                                            row.priceDisplay += " (If you buy from "+min+" to "+max+" products, you can purchase each product with this option for only "+symbol+row.currency_symbol+price+" per product.)"
                                        }
                                    });
                                }else {
                                    row.priceDisplay = row.formatted_special_price;
                                ...
                                if (row.tiersi !== undefined) {
                                    ko.utils.arrayForEach(row.tiersi, function (tier) {
                                        ...
                                        if(type == 'percentage'){
                                            row.priceDisplay += " (If you buy from "+min+" to "+max+" products, you can purchase each product with this option for only "+symbol+price+"% per product.)"
                                        }else {
                                            row.priceDisplay += " (If you buy from "+min+" to "+max+" products, you can purchase each product with this option for only "+symbol+row.currency_symbol+price+" per product.)"
                                        }
                                    });
